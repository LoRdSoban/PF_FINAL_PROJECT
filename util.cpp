...
void DabbaBanao(int x1, int y1 ,int x2, int y2, float *color)
{
	glColor3fv(color);
	glBegin(GL_POLYGON);
	glVertex2s( x1, y1 );
 ...
 void DabbaBanaoNEW(int xy1, int xy2 , int xy ,int d, float *color)
 {
	...
	//horizontal road
 	if (d == 1)
 	{
 		glVertex2s( xy1*32, xy*32 );
 		glVertex2s( xy2*32, xy*32 );
 		glVertex2s( xy2*32, (xy+1)*32 );
 		glVertex2s( xy1*32, (xy+1)*32 );
 	}
	//vertical road
	else if (d == 2)
	{															
		glVertex2s( xy*32, xy1*32);
 		glVertex2s( (xy+1)*32, xy1*32);
 		glVertex2s( (xy+1)*32, xy2*32);
 		glVertex2s( xy*32, xy2*32);	
	}
 	// glVertex2s( x1, y1 );
 	// glVertex2s( x2, y1 );
 	// glVertex2s( x2, y2 );
